...
if (!function_exists('frontend_asset_version')) {
    /**
     * Cache-busting version for a public asset path.
     */
    function frontend_asset_version($path)
    {
        static $cache = [];
        ...
        $globalVersion = config('app.asset_version');
        if (!empty($globalVersion)) {
            return $cache[$path] = (string) $globalVersion;
        }

        $fullPath = public_path($path);
        if (is_file($fullPath)) {
            return $cache[$path] = (string) filemtime($fullPath);
        }
...
if (!function_exists('frontend_asset')) {
    /**
     * Versioned URL for theme CSS/JS under public/ (supports R2 CDN).
     */
    function frontend_asset($path)
    {
        ...
        return $url . '?v=' . frontend_asset_version($path);
    }
}
