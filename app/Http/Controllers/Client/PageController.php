...
class PageController extends Controller
{
    ...
    public function postcontact(Request $request){
        ...
        $data->mess = $request->mess ?: $request->message ?: null;

        $data->service_name = $request->service_name ?: $request->service_pick ?: null;
        ...
        if (!$data->service_slug && $data->service_cate_slug) {
            $data->service_slug = $data->service_cate_slug;
        }
    ...
    public function serviceList($slug)
    {
        ...
        $data['itineraries'] = Itinerary::where('status', 1)->orderBy('sort')->orderBy('id')->get();
    ...
    public function haitrinh()
    {
        $data['itineraries'] = Itinerary::where('status', 1)->orderBy('sort')->orderBy('id')->get();
        return view('haitrinh',$data);
    }

    public function haitrinhDetail($slug)
    {
        $detail = Itinerary::where('slug', $slug)->where('status', 1)->firstOrFail();
        $data['detail'] = $detail;
        $data['itineraries'] = Itinerary::where('status', 1)
            ->orderBy('sort')
            ->orderBy('id')
            ->get(['id', 'name', 'slug']);
        return view('haitrinhDetail', $data);
    }
}
