 ...
 *   data-cbw-phone, data-cbw-facebook, data-cbw-zalo, data-cbw-messenger
...
(function (global) {
    ...
    function normalizeMessengerUrl(value) {
        ...
        if (!trimmed) return '';
        if (/^https?:\/\//i.test(trimmed)) return trimmed;
        if (/^(m\.me|www\.messenger\.com|messenger\.com)/i.test(trimmed)) {
            return 'https://' + trimmed.replace(/^\/+/, '');
        }
        var slug = trimmed.replace(/^@/, '').replace(/^\/+/, '');
    ...
    function parseConfig(root) {
        return {
            ...
            zalo: root.getAttribute('data-cbw-zalo') || root.getAttribute('data-cbw-instagram') || '',
            messenger: root.getAttribute('data-cbw-messenger') || root.getAttribute('data-cbw-tiktok') || '',
    ...
    function applyConfig(root, config) {
        ...
        if (config.zalo) root.setAttribute('data-cbw-zalo', config.zalo);
    ...
    function syncLinks(root) {
        ...
        bindItem(root, '[data-cbw-link="zalo"]', normalizeZaloUrl(cfg.zalo), !!cfg.zalo);
        bindItem(root, '[data-cbw-link="messenger"]', normalizeMessengerUrl(cfg.messenger), !!cfg.messenger);
